# Shellty Lingo — wytyczne inżynieryjne (pełny dokument referencyjny)

> Ten dokument zawiera pełne wytyczne techniczne projektu. Zwięzły plik kontekstu
> dla programistów i agentów znajduje się w [`CLAUDE.md`](../CLAUDE.md) w katalogu
> głównym; odwołania w kodzie w formie „§13" wskazują sekcje tego dokumentu.

## 1. Przeznaczenie dokumentu

Dokument jest pełnym technicznym punktem odniesienia projektu. Ma umożliwić programiście lub agentowi programistycznemu zrozumienie produktu, granic MVP, architektury, konwencji i kryteriów jakości bez ponownego analizowania materiałów źródłowych.

Dokument opisuje stan docelowy. Jeśli kod, migracje lub zatwierdzony ADR są sprzeczne z tym dokumentem, obowiązuje kolejność:
...
- słownik kontekstowy z tłumaczeniem słów, zwrotów i treści zadań, odczytem TTS tekstu źródłowego oraz tłumaczenia i zapisem do powtórek;
...
- Dostęp do zewnętrznego tłumaczenia i TTS przechodzi przez backend oraz wymienne adaptery. Klient może użyć głosu systemowego tylko jako kontrolowanego fallbacku; nie przechowuje kluczy dostawcy.
...
- `VocabularyItem`, `DictionaryEntry`, `PronunciationVariant`, `TtsAsset`, `GrammarTopic`, `MediaAsset`, `Translation`;
...
- `VocabularyProgress`, `SavedVocabularyItem`, `DictionaryLookup`, `ReviewSchedule`, `SkillProgress`;
...
### Słownik kontekstowy, tłumaczenie i TTS

- użytkownik może otworzyć słownik bez opuszczania lekcji, ćwiczenia lub rozmowy AI przez dotknięcie albo zaznaczenie słowa, zwrotu, zdania bądź treści zadania;
- słownik pokazuje tekst źródłowy, tłumaczenie na wybrany język wyjaśnień, znaczenie właściwe dla kontekstu, część mowy i przykład użycia, jeśli są dostępne;
- dla tajskiego pokazuje również zapis tajski, transliterację, oznaczenie tonu, wariant grzecznościowy i zweryfikowaną wymowę, jeśli dane je zawierają;
- przycisk odsłuchu odczytuje osobno tekst źródłowy i jego tłumaczenie; użytkownik może zatrzymać, ponowić odtwarzanie oraz wybrać co najmniej kilka prędkości mowy;
- wynik można zapisać do własnego słownika i kolejki spaced repetition wraz z kontekstem, z którego pochodzi;
- pierwszeństwo mają recenzowane wpisy i nagrania kursowe; tłumaczenie lub audio wygenerowane dynamicznie musi być odpowiednio oznaczone i nie może automatycznie stać się opublikowaną treścią edukacyjną;
- tłumaczenie dłuższej treści zadania nie może ujawniać poprawnej odpowiedzi, chyba że użytkownik jawnie poprosi o pełne tłumaczenie i produkt zarejestruje użycie podpowiedzi;
- TTS i tłumaczenie mają timeout, limit długości, kontrolę kosztu, cache wyłącznie dla bezpiecznej treści niepersonalnej oraz fallback, który nie blokuje wykonania zadania;
- brak obsługi głosu, sieci lub danego języka jest komunikowany czytelnie; aplikacja nie deklaruje poprawnej wymowy, jeśli dostępny głos nie przeszedł kontroli jakości;
- treści EN/TH, kierunki tłumaczeń do PL/EN/TH oraz wymowa tajska są objęte testami regresji i okresowym przeglądem ekspertów.
...
- zaznaczenie tekstu i panel słownika nie mogą resetować odpowiedzi, timera ani postępu bieżącego ćwiczenia;
- odtwarzacz TTS udostępnia osobne kontrolki dla oryginału i tłumaczenia, zatrzymuje poprzedni odsłuch przed uruchomieniem kolejnego i respektuje systemowe ustawienia audio oraz dostępności;
...
- słownik z lekcji/ćwiczenia/rozmowy → tłumaczenie słowa i pełnej treści → TTS obu wersji → zapis do własnych słów → pojawienie się w powtórkach;
...
- AI/tłumaczenie/TTS: timeout, status, walidacja schematu, użyty język i głos, jednostki użycia, cache hit oraz koszt;
...
3. dostawcy chmury, AI, tłumaczenia, TTS, e-mail, analityki, monitoringu i płatności oraz jakość głosów EN/TH/PL;
